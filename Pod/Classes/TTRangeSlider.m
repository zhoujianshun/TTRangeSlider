...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    if (self.leftHandleSelected){
        if (self.enableStep && self.step>=0.0f) {
            if (self.stepContinues) {
                CGFloat x = self.selectedMinimum;
                CGFloat distance = self.step;
        ...
        if (self.enableStep && self.step>=0.0f) {
            if (self.stepContinues) {
                CGFloat x = self.selectedMaximum;
                CGFloat distance = self.step;
...
- (void)setDisableRange:(BOOL)disableRange {
    ...
    if (_disableRange){
        ...
        self.sliderLineBetweenHandles.cornerRadius = 0;
    }
}
